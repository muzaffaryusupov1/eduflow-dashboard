...
import { useUpdateGroupStatus } from "../queries"
  ...
  return (
    <Card className="p-0">
      ...
      <CardContent className="p-0">
        {isError ? (
          <div className="p-6 text-sm text-destructive">Failed to load groups.</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
              ...
              {!isLoading &&
                rows.map((group) => (
                  <TableRow key={group.id} className="cursor-pointer">
                    ...
                    <TableCell onClick={() => onRowClick(group.id)}>
                      {group.teacher?.fullName ?? group.teacher?.email ?? "—"}
                    </TableCell>
